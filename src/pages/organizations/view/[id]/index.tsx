...
import { deleteBrandById } from 'apiSdk/brands';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'brand', 'customer'],
      }),
  );

  const brandHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('brand', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/brands/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.brand?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/brands/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            <Td>
