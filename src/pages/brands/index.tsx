...
function BrandListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BrandInterface[]>(
    ...
    () =>
      getBrands({
        relations: ['organization', 'equipment.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('brand', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/brands/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('brand', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/brands/create`} passHref legacyBehavior>
